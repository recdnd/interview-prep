...
- **`[xx]（例: …）` プレースホルダ**を本文に埋め込む pack（例：統合後の **`smart-full-lines`** 内 `〔hard-work〕` 題）と、Cursor 向けの置換・記録規範は **[hard-work-placeholder-notes.md](./hard-work-placeholder-notes.md)** にまとめる（題庫ファイル外のメモとして保守する想定）。
