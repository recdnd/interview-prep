...
function initApp(PACK, currentPack, PACK_ORDER, PACK_LABEL) {
  ...
  const CORPUS = window.HELPER_CORPUS;
  if (!CORPUS || !CORPUS.cheat || !CORPUS.cbl || !CORPUS.aff) {
    throw new Error(
      'HELPER_CORPUS が未定義、または cheat/cbl/aff が欠けています。helpers/helper-corpus.js を script.js より先に読み込んでください。'
    );
  }
  const CHEAT = CORPUS.cheat;
  const CBL = CORPUS.cbl;
  const AFF = CORPUS.aff;
